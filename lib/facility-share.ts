 ...
 */
export const COPY_DISCLAIMER: Record<Lang, { basis: string; staleness: string; source: string }> = {
  ko: {
    basis: "공식 등록 영업시간 기준이며, 지금 문을 열었는지 확인한 것이 아닙니다",
    staleness: "공식 자료가 갱신되기 전이면 이미 퇴점한 매장이 남아 있을 수 있습니다",
  ...
  ja: {
    ...
    staleness: "公式資料が更新されるまで、すでに退店した店舗が残っている場合があります",
...
export function buildFacilityCopyText(facility: CopyableFacility, lang: Lang, labels: CopyLabels): string {
  ...
  lines.push("", `※ ${disclaimer.basis}`, `※ ${disclaimer.staleness}`, disclaimer.source);
  return lines.join("\n");
}
