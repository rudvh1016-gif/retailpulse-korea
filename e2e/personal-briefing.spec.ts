...
for(const lang of ['ko','en','zh','ja'] as const) for(const width of [390,768,1280,1920]) {
  test(`personal setup and briefing ${lang} ${width}`,async({page})=>{
    await page.setViewportSize({width,height:900});await fixture(page);await page.goto(`/${lang}`);await page.locator('.personal-existing > summary').click();
    ...
    await page.reload();await page.locator('.personal-existing > summary').click();await expect(page.getByTestId('personal-briefing')).toBeVisible();
...
test('reopening with persisted browser state, editing, feedback and reset',async({page,browser})=>{
  await fixture(page);await page.goto('/ko');await page.locator('.personal-existing > summary').click();await setup(page);
  const state=await page.context().storageState();
  const context=await browser.newContext({storageState:state});const reopened=await context.newPage();
  await fixture(reopened);await reopened.goto('/ko');await reopened.locator('.personal-existing > summary').click();await expect(reopened.getByTestId('personal-briefing')).toBeVisible();
  await reopened.getByRole('button',{name:'도움됐어요',exact:true}).click();
  await reopened.reload();await reopened.locator('.personal-existing > summary').click();await expect(reopened.getByRole('button',{name:'도움됐어요',exact:true})).toBeDisabled();
...
test('multiple locations, terminals and all three days persist and switch to the matching date',async({page})=>{
  await page.setViewportSize({width:390,height:900});await fixture(page);await page.goto('/ko');await page.locator('.personal-existing > summary').click();
  ...
  await page.reload();await page.locator('.personal-existing > summary').click();
...
test('settings survive closing and relaunching a persistent browser',async({},testInfo)=>{
  ...
  let page=await context.newPage();await fixture(page);await page.goto('http://127.0.0.1:4173/ko');await page.locator('.personal-existing > summary').click();await setup(page);
  await context.close();
  context=await chromium.launchPersistentContext(profile,{headless:true});
  page=await context.newPage();await fixture(page);await page.goto('http://127.0.0.1:4173/ko');await page.locator('.personal-existing > summary').click();
