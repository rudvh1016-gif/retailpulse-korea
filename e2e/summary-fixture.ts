...
/**
 * Seoul city-data context for 명동. Exported on its own so a spec can vary one
 * field — a stale observation, a single published category — without going
 * through AREA_BLOCK, whose `Record<string, unknown>` overrides erase the type.
 */
export const MYEONGDONG_CONTEXT = {
  ...
  weather: {
    // Five minutes before generatedAt (05:10Z). It used to be 14:50 —
    // forty minutes AFTER the summary was generated — which no real
    // observation can be, and which the freshness rules correctly
    // refuse to call "지금".
    observedAt: "2026-08-31T14:05:00+09:00", temperature: 29.4, humidity: 44, wind: 5,
...
export const SUMMARY_FIXTURE = {
  ...
  areas: {
    myeongdong: AREA_BLOCK({
      ...
      // Humidity and wind included: without a wind reading the guide line
      // correctly omits its wind half, which is not the shape a reader sees.
      weather: [{ targetAt: "2026-08-31T18:00:00+09:00", precipitationProbability: 60, temperatureTenthC: 270, conditionCode: "rain", humidityPercent: 65, windSpeedTenthMps: 25 }],
      subwayRidership: {
