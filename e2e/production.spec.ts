...
test("departure composition is one accessible tab group with gate, airline and registered-country views", async ({ page }) => {
  ...
  /*
   * The country label is whatever THIS browser's CLDR data calls KR, so it
   * cannot be hardcoded: Chromium's short name for KR is "한국" on some
   * builds and "대한민국" on others, and `lib/airline-ranking.ts` already
   * notes the same CLDR variance for HK/MO. Hardcoding either string makes
   * the suite pass on one machine and fail on the next — which is exactly
   * what it was doing, red on main before this branch. Asking the page's
   * own `Intl` keeps the assertion meaningful (the label really is KR's
   * name) and true on every runner.
   */
  const koreaLabel = await page.evaluate(() =>
    new Intl.DisplayNames(["ko-KR"], { type: "region", style: "short", fallback: "code" }).of("KR") ?? "KR");
  expect(koreaLabel).not.toBe("KR");
  await expect(rows.nth(0)).toContainText(koreaLabel);
  ...
  await expect(countries.nth(0)).toContainText(koreaLabel);
