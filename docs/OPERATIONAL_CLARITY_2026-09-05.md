...
2. The full physical-flight set is partitioned into T1, T2, other provider labels, no terminal label and conflicting labels. Missing labels are never inferred as concourse/T1/T2 from gate numbers. A capped query is explicit. Existing stored terminal mappings are untouched.
