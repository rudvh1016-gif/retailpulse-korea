...
standard Seoul Open Data paging segments. A bounded 2026-09-03 public sample
probe returned `INFO-200` for unpublished `20263` and `INFO-000` for latest
published quarter `20262`. The successful response exposed these fields:
...
store count, multiplied by 100. Across the first five public sample rows for
each mapped area, all 30 opening/closure rate checks matched integer rounding
of that formula. This bounded sample check does not stand in for the collector's
full-row validation. The response has no source-update timestamp field. The
public metadata currently identifies the source as annual-refresh, slow-moving
data and states that records from 2024 use the standard-unit-area basis.

## Versioned geographic mapping

Mapping version: `oa-15577-standard-area-2026-09-03-v1`.

| Product area | OA-15577 code | Current official name | Type | Official type name | Rule |
|---|---|---|---|---|---|
| Myeongdong | `3001492` | 명동 남대문 북창동 다동 무교동 관광특구 | `U` | 관광특구 | one official area only |
| Hongdae | `3120103` | 홍대입구역(홍대) | `D` | 발달상권 | one official area only |
| Seongsu | `3110131` | 성수동카페거리 | `A` | 골목상권 | one official area only |

All three mappings returned `INFO-000` with the exact code, name, type, and
`20262` period in the current OA-15577 sample contract. Reported row counts
were 88, 90, and 68 respectively; the contract check did not enumerate the
full rows with an authenticated key. The mappings also match the one-primary-area
...
exactly one compact row per product area and quarter. The area total is a
KORETAIL-derived sum of unique OA-15577 industry rows, not a provider-published
single area-total row or a claim to enumerate every legal business. It stores:
...
Transport calls intentionally use one attempt so the maximum remains literal;
weekly Last-good behavior is preferred to quietly exceeding the bounded
provider-call contract. Writes are semantic changed-only upserts. A failed, empty, mismatched, or
malformed response writes no store row and preserves last-good data. Source
health becomes `STALE` only when all three exact mappings have semantically
valid Last-good rows from one quarter; otherwise it becomes `ERROR`. Success
is `OFFICIAL_HISTORICAL`.
