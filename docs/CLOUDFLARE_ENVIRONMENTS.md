...
- Production carries exactly five Cron Triggers, all **trigger-only**. Three are the primary cadences — `7,22,37,52 * * * *`, `42 * * * *`, `10 2,5,8,11,14,17,20,23 * * *` — routing to `collect-realtime.yml`, `collect-forecast.yml` and `collect-weather.yml`. Two are recovery expressions — `53 * * * *` and `25,40 2,5,8,11,14,17,20,23 * * *` — routing to `collect-forecast-recovery.yml` and `collect-weather-recovery.yml`; the weather one fires twice per issuance, at :25 and :40, from that single expression. The handler matches the exact Cron expression and makes one authenticated GitHub `workflow_dispatch` call; it performs no provider call, hashing or D1 access (`docs/REALTIME_SCHEDULER_AUDIT.md`). Staging and the default environment remain Cron-free.
- **Workers Free allows at most 5 Cron Triggers per account**, so the list above is full. A sixth does not degrade gracefully: the Cloudflare API rejects the whole `/schedules` update with code 10072 after the Worker script has uploaded, failing the deploy while leaving the previous schedule in place. Adding a sixth *expression* now means removing one, or moving to Workers Paid. Adding a firing time to an expression already configured does not: the cap counts expressions, and the minute field accepts a comma list, which is how weather recovery covers :25 and :40 within the five.
- No two triggers fire on the same minute. A5 recovery is `53` rather than `52` because the realtime cadence already fires at `:52`, and two expressions matching one minute is an ambiguity the routing must not have; realtime cadence itself is unchanged.
