...
- Production carries exactly six Cron Triggers, all **trigger-only**. Three are the primary cadences — `7,22,37,52 * * * *`, `42 * * * *`, `10 2,5,8,11,14,17,20,23 * * *` — routing to `collect-realtime.yml`, `collect-forecast.yml` and `collect-weather.yml`. Three are recovery windows — `53 * * * *`, `25 2,5,8,11,14,17,20,23 * * *`, `40 2,5,8,11,14,17,20,23 * * *` — routing to `collect-forecast-recovery.yml` and `collect-weather-recovery.yml`. The handler matches the exact Cron expression and makes one authenticated GitHub `workflow_dispatch` call; it performs no provider call, hashing or D1 access (`docs/REALTIME_SCHEDULER_AUDIT.md`). Staging and the default environment remain Cron-free.
