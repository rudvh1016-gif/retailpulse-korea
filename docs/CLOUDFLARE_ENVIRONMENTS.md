...
- Production carries exactly one Cron Trigger, `7,22,37,52 * * * *`, and it is **trigger-only**: the handler makes one authenticated GitHub `workflow_dispatch` call to `collect-realtime.yml` and performs no provider call, hashing or D1 access (`docs/REALTIME_SCHEDULER_AUDIT.md`). Staging and the default environment remain Cron-free.
