...
- Production carries exactly five Cron Triggers, all **trigger-only**. Three are the primary cadences — `7,22,37,52 * * * *`, `42 * * * *`, `10 2,5,8,11,14,17,20,23 * * *` — routing to `collect-realtime.yml`, `collect-forecast.yml` and `collect-weather.yml`. Two are recovery windows — `53 * * * *` and `25 2,5,8,11,14,17,20,23 * * *` — routing to `collect-forecast-recovery.yml` and `collect-weather-recovery.yml`. The handler matches the exact Cron expression and makes one authenticated GitHub `workflow_dispatch` call; it performs no provider call, hashing or D1 access (`docs/REALTIME_SCHEDULER_AUDIT.md`). Staging and the default environment remain Cron-free.
- **Workers Free allows at most 5 Cron Triggers per account**, so the list above is full. A sixth does not degrade gracefully: the Cloudflare API rejects the whole `/schedules` update with code 10072 after the Worker script has uploaded, failing the deploy while leaving the previous schedule in place. Adding a cadence now means removing one, or moving to Workers Paid.
