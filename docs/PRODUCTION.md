...
- Production Worker Cron carries **no collector work**. Since 2026-08-31 it holds exactly one **trigger-only** Cron (`7,22,37,52 * * * *`) whose handler makes a single authenticated GitHub `workflow_dispatch` call; heavy Cron execution of collectors stays rejected by the benchmark in `docs/REALTIME_SCHEDULER_AUDIT.md`.
...
- Cadence is split into five independently scheduled groups, each its own workflow file, all gated behind the same repository-level `ENABLE_PRODUCTION_COLLECTOR` switch and never overlapping on the same source (see `docs/DATA_SOURCES.md` and `tests/hybrid.test.ts` for the enforced one-owner-per-source coverage check):
  ...
  - **REALTIME** (`collect-realtime.yml`, `:07/:22/:37/:52`, ~15 min): `airport_congestion` (A4-T1), `airport_congestion_t2` (A4-T2), `seoul_realtime` (S1). This group alone is scheduled by the Cloudflare production Cron rather than a GitHub `schedule:` block — the cadence is identical, the workflow keeps `workflow_dispatch`, and its runs therefore appear as `event=workflow_dispatch`.
