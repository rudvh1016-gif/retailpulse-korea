...
The 150-page hard bound means at most 150 A1 calls for one manual run. A1's public catalog currently documents 500 development calls/day. This fallback therefore must remain manual while the development quota applies. It is not approved for recurring collection. If the exact official date/time parameters are later verified, replace this full-window fallback with the narrower official query and re-audit quota/cadence before enabling any schedule.
