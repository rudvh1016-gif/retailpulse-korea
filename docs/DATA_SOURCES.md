...
- Official data refresh: ~5 minutes. KORETAIL polls once/hour (`.github/workflows/collect-forecast.yml`, cron `:42` — deliberately off every other collector group's minute marks). Normal cost ≈2 requests/cycle (today + tomorrow, each usually fitting in one page against the official sample `totalCount=25`) × 24 runs/day ≈ 48 requests/day, with bounded per-day pagination if `totalCount` ever exceeds one page.
