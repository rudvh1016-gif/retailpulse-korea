    ...
    <div className={"app lang-" + lang}>
      ...
      <nav className="bottom-nav" aria-label="Primary">
        {(["today", "airport", "business", "forecast", "more"] as View[]).map((item) => (
          <button key={item} className={view === item ? "active" : ""} onClick={() => navigate(item)} aria-current={view === item ? "page" : undefined}>
            <Icon name={item} />
...
function MoreView({
  ...
  return (
    <section className="view-section more-view">
      ...
      <section className="privacy-note" id="privacy" aria-labelledby="privacy-title">
      ...
      <nav className="docs-grid" aria-label="Production documents">
        {[{ href: "/production-handoff.md", label: t.handoff }, { href: "/growth-validation-plan.md", label: "FORECAST & ACQUISITION PLAN" }, { href: "/api-key-audit.md", label: "API KEY & FREE-USE AUDIT" }, { href: "/seo-handoff.md", label: "SEO HANDOFF" }, { href: "/product-roadmap.md", label: "PRODUCT ROADMAP" }, { href: "/data-source-matrix.md", label: "DATA SOURCE MATRIX" }, { href: "/gate-retail-data-audit.md", label: "GATE & RETAIL DATA AUDIT" }, { href: "/historical-backfill-plan.md", label: "HISTORICAL BACKFILL PLAN" }, { href: "/qa-report-v5.md", label: "100-POINT QA REPORT" }, { href: "/qa-report-v5-6.md", label: "V5.6 RANGE & API QA" }, { href: "/qa-report-v5-7.md", label: "V5.7 CREDENTIAL & CUSTOMER QA" }, { href: "/qa-report-v5-8.md", label: "V5.8 EVIDENCE & GROWTH QA" }, { href: "/live-readiness.md", label: "LIVE READINESS" }].map((item) => <a className="handoff-link" href={item.href} target="_blank" rel="noreferrer" key={item.href}><span><small>BUILD NOTE</small>{item.label}</span><b>↗</b></a>)}
      </nav>
    </section>
  );
}
