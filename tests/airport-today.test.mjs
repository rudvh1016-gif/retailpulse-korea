...
test("A1 today fallback scans every bounded 100-row page, filters KST date, and deduplicates codeshares", async () => {
  const seen = [];
  const pages = new Map([
    [1, [flight({ flightId: "KE100", scheduleDatetime: "202608290900" })]],
    [2, [
  ...
  assert.deepEqual(result.records.map((record) => record.flightNumber).sort(), ["KE200", "OZ300"]);
